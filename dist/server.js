...
function normalizeDatabricksAuthEnv() {
	const host = process.env.DATABRICKS_HOST?.trim();
	if (host && !/^https?:\/\//i.test(host)) process.env.DATABRICKS_HOST = `https://${host}`;
	if (process.env.DATABRICKS_HOST && process.env.DATABRICKS_TOKEN) delete process.env.DATABRICKS_CONFIG_PROFILE;
}
normalizeDatabricksAuthEnv();
createApp({
	plugins: [server({ bodyLimit: "12mb" }), genie({ spaces: { default: "01f168ebcf2c14e892b5843e00a085d6" } })],
