...
function registerCareFinderRoutes(app) {
	...
	app.post("/api/care-finder/geocode", async (req, res) => {
		try {
	...
	app.post("/api/care-finder/reverse-geocode", async (req, res) => {
		try {
	...
	app.get("/api/care-finder/recent", async (_req, res) => {
		try {
	...
	app.post("/api/care-finder/analyze", async (req, res) => {
		try {
			const response = await analyzeAndRecommend(parseAnalyzeRequest(req.body));
			res.json(response);
		} catch (error) {
			sendRouteError(res, error);
		}
...
async function analyzeAndRecommend(request) {
	...
	const primaryFacilityId = recommended.length > 0 ? stringFromUnknown(recommended[0].unique_id ?? recommended[0].name) : "";
	let writeWarning = "";
	try {
		await insertResult({
			imageId,
...
async function runFacilityMatchModel(parsed, userContext, candidates) {
	if (candidates.length === 0) return {
		matches: [],
		verification: { summary: "No candidates to rerank." }
	};
...
Score each candidate from 0 to 100. Include every candidate exactly once.`;
	const parsedResponse = safeJsonLoads(extractMessageContent(await callAiGateway({
	...
	if (!Array.isArray(parsedResponse.matches)) parsedResponse.matches = [];
...
async function loadFacilities(userLat, userLon, maxDistanceKm) {
	...
	const lonDelta = Math.max(maxDistanceKm / (111 * Math.max(Math.cos(Math.PI * userLat / 180), .2)), .25);
...
async function executeSql(statement, parameters) {
	...
	for (let attempt = 0; attempt < 10 && response.status?.state !== "SUCCEEDED"; attempt += 1) {
		if (response.status?.state === "FAILED" || response.status?.state === "CANCELED" || response.status?.state === "CLOSED") throw new Error(response.status.error?.message ?? `SQL statement ended with state ${response.status.state}`);
		if (!response.statement_id) throw new Error("SQL statement did not return a statement_id for polling.");
		await delay(1e3);
		response = await client.statementExecution.getStatement({ statement_id: response.statement_id });
	}
	if (response.status?.state !== "SUCCEEDED") throw new Error(`SQL statement did not finish. Current state: ${response.status?.state ?? "unknown"}`);
	const columns = response.manifest?.schema?.columns?.map((column) => column.name ?? "") ?? [];
	return (response.result?.data_array ?? []).map((row) => Object.fromEntries(columns.map((column, index) => [column, parseSqlValue(row[index])])));
}
function getWorkspaceClient() {
	workspaceClient ??= new WorkspaceClient({});
	return workspaceClient;
}
function recommendFacilities(facilities, userLat, userLon, parsed, settings, topN) {
	...
	for (const row of facilities) {
		...
		if (distanceKm > settings.maxDistanceKm) continue;
		...
		if (settings.newPatientsFilter === "Only facilities taking new patients" && newPatientStatus !== "accepting") continue;
		if (settings.newPatientsFilter === "Only facilities with unknown status" && newPatientStatus !== "unknown") continue;
		if (settings.insuranceFilter === "Insurance accepted/mentioned" && insuranceStatus !== "insurance_mentioned") continue;
		if (settings.insuranceFilter === "Self-pay/cash accepted/mentioned" && insuranceStatus !== "self_pay_mentioned") continue;
		if (settings.insuranceFilter === "Unknown" && insuranceStatus !== "unknown") continue;
		rows.push({
			...row,
			distance_km: round(distanceKm, 2),
			relevance_score: scoreDetails.score,
			combined_score: scoreDetails.score * 1e3 - distanceKm,
...
async function rerankWithFoundationModel(baselineCandidates, parsed, userContext, topN, userAddressOrNotes, userSymptoms, userName, appointmentPreference) {
	if (baselineCandidates.length === 0) return [];
	...
	for (const match of matches) if (isRecord(match)) matchById.set(stringFromUnknown(match.candidate_id), match);
	return baselineCandidates.map((row, index) => {
		...
		const enriched = {
			...
			fm_missing_or_uncertain: Array.isArray(match.missing_or_uncertain_capabilities) ? match.missing_or_uncertain_capabilities.map(String).filter(Boolean).join(", ") : stringFromUnknown(match.missing_or_uncertain_capabilities),
		...
		enriched.whatsapp_chat_url = buildWhatsappUrl(phone, buildWhatsappMessage(enriched, parsed, userAddressOrNotes, userSymptoms, userName, appointmentPreference, "chat"));
		enriched.whatsapp_appointment_url = buildWhatsappUrl(phone, buildWhatsappMessage(enriched, parsed, userAddressOrNotes, userSymptoms, userName, appointmentPreference, "appointment"));
		return enriched;
	}).sort((a, b) => {
		const fmDiff = Number(b.fm_match_score ?? 0) - Number(a.fm_match_score ?? 0);
		if (fmDiff !== 0) return fmDiff;
		const relevanceDiff = Number(b.relevance_score ?? 0) - Number(a.relevance_score ?? 0);
		if (relevanceDiff !== 0) return relevanceDiff;
		return Number(a.distance_km ?? 0) - Number(b.distance_km ?? 0);
	}).slice(0, topN);
}
function compareTaxonomyVsFm(taxonomyRanked, fmRanked) {
	if (taxonomyRanked.length === 0 || fmRanked.length === 0) return { status: "insufficient_candidates" };
	...
	return {
		...
		verification_note: "FM reranking adds a semantic check over taxonomy keywords by reading illness context against specialties, procedures, equipment, capabilities, and descriptions. This is a model-as-judge runtime verification, not labeled clinical ground truth."
	};
}
function buildCareTerms(parsed) {
	const terms = [];
	for (const key of [
		"taxonomy_terms",
	...
	if (urgency === "emergency" || urgency === "urgent") terms.push("emergency", "trauma", "hospital", "critical care", "x-ray", "laboratory", "icu");
	return [...new Set(terms.map((term) => term.trim()).filter(Boolean))];
}
function inferCareDomains(parsed) {
	const evidenceParts = [];
	for (const key of [
		"care_domains",
		"taxonomy_terms",
	...
	for (const [domain, config] of Object.entries(CARE_TAXONOMY)) {
		let score = 0;
		for (const keyword of config.keywords) score += scoreTermAgainstBlob(evidence, keyword, 5, 1);
		if ((urgency === "emergency" || urgency === "urgent") && config.boost_when_urgent) score += 8;
		if (score > 0) scored.push([domain, score]);
	}
	return scored.sort((a, b) => b[1] - a[1]).slice(0, 5).map(([domain]) => domain);
}
function getCareDomains(parsed) {
	const fromParsed = Array.isArray(parsed.care_domains) ? parsed.care_domains.map(String).filter((domain) => CARE_TAXONOMY[domain]) : [];
	return fromParsed.length > 0 ? fromParsed : inferCareDomains(parsed);
}
function keywordScore(row, careTerms, careDomains) {
	const fieldWeights = {
		specialties: 8,
		procedure: 7,
		equipment: 7,
	...
	for (const domain of careDomains) {
		...
		if (!config) continue;
		let domainScore = 0;
		for (const keyword of config.keywords) for (const [field, weight] of Object.entries(fieldWeights)) {
			const fieldValue = row[field];
			if (fieldValue === void 0 || fieldValue === null) continue;
	...
	if (blob.includes("hospital")) score += 1;
	if (blob.includes("emergency") || blob.includes("24/7") || blob.includes("24x7") || blob.includes("trauma")) score += 4;
	if (blob.includes("multispecialty") || blob.includes("multi specialty") || blob.includes("super specialty")) score += 3;
	return {
...
function facilityBlob(row) {
	return [
		...
		"appointment",
		"source_urls"
	].map((field) => row[field]).filter((value) => value !== void 0 && value !== null).map((value) => stringFromUnknown(value).toLowerCase()).join(" ");
}
function inferNewPatientStatus(row) {
	...
	if ([
		...
		"no new patients"
	].some((term) => blob.includes(term))) return "not_accepting";
	if ([
	...
	].some((term) => blob.includes(term))) return "accepting";
	return "unknown";
}
function inferInsuranceStatus(row) {
	const blob = facilityBlob(row);
	const hasInsurance = [
		"insurance",
		"insured",
	...
	if (hasInsurance) return "insurance_mentioned";
	if (hasSelfPay) return "self_pay_mentioned";
	return "unknown";
}
function textBlob(row) {
	return [
		...
		...TAXONOMY_FIELDS
	].map((field) => row[field]).filter((value) => value !== void 0 && value !== null).map(normalizeSearchText).join(" ");
}
function scoreTermAgainstBlob(blob, term, exactWeight, tokenWeight) {
	const termClean = normalizeSearchText(term).trim();
	if (!termClean) return 0;
	let score = blob.includes(termClean) ? exactWeight : 0;
	for (const token of termClean.split(/\s+/)) if (token.length >= 4 && blob.includes(token)) score += tokenWeight;
	return score;
}
function buildWhatsappMessage(row, parsed, userAddressOrNotes, userSymptoms, userName, appointmentPreference, messageKind) {
	return [
		messageKind === "appointment" ? "Hello, I would like to request an appointment." : "Hello, I found your facility through a care finder app and would like to ask about availability.",
...
function getFacilityWhatsappPhone(row) {
	for (const field of [
		"officialPhone",
		"phone_numbers",
		"phone",
...
function extractFirstPhoneNumber(value) {
...
function makeAddress(row) {
	return [
		...
		"address_country"
	].map((field) => row[field]).filter((value) => value !== void 0 && value !== null && stringFromUnknown(value).trim()).map((value) => stringFromUnknown(value).trim()).join(", ");
}
function parseSourceUrls(value) {
	return parseListLike(value).filter(Boolean).slice(0, 3).join(", ");
}
async function reverseGeocodeLatLon(lat, lon) {
	...
	return {
		label: compactLocationLabel(address) || stringFromUnknown(responseRecord.display_name),
...
async function geocodeCityTownState(placeText) {
	const query = placeText.trim();
	if (!query) return { error: "Enter a city, town, or state." };
	...
	const first = (Array.isArray(response) ? response : []).find(isRecord);
	if (!first) return { error: `Could not find coordinates for '${query}'.` };
...
async function fetchUrlJson(url, params) {
	...
	const response = await fetch(requestUrl, { headers: { "User-Agent": "care-finder-vision-app/1.0" } });
	if (!response.ok) throw new Error(`Geocoding request failed (${response.status}): ${response.statusText}`);
	return response.json();
}
function compactLocationLabel(address) {
	return [
		getCityOrTown(address),
		address.state ?? address.state_district ?? "",
		address.country ?? ""
	].map((value) => stringFromUnknown(value)).filter(Boolean).join(", ");
}
function getCityOrTown(address) {
	return stringFromUnknown(address.city ?? address.town ?? address.village ?? address.municipality ?? address.suburb ?? address.county ?? "");
}
function taxonomyContextForPrompt() {
	return JSON.stringify(Object.fromEntries(Object.entries(CARE_TAXONOMY).map(([domain, config]) => [domain, {
		label: config.label,
...
function facilityCandidatePayload(row, candidateId) {
	return {
...
function normalizeSearchText(value) {
	return splitCamelCase(parseListLike(value).join(" ")).replace(/[_-]/g, " ").toLowerCase();
}
...
function safeJsonLoads(text) {
	...
	if (cleaned.startsWith("```")) cleaned = cleaned.replace(/^```(?:json|JSON)?\s*/, "").replace(/```$/, "").trim();
...
function extractMessageContent(response) {
	const firstChoice = (Array.isArray(response.choices) ? response.choices : []).find(isRecord);
	const content = (isRecord(firstChoice?.message) ? firstChoice.message : {}).content;
...
function getNumber(record, key, fallback) {
...
//#endregion
export { registerCareFinderRoutes };